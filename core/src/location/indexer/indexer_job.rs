...
impl Job for IndexerJob {
	...
	fn run(&self) -> Box<dyn Stream<Item = JobMessage>> {
		stream! {
			init_indexer_job().await;

			// TODO
			// for step in self.steps {

				// run_indexer_step().await;

				// TODO: Pop from state once complete
			// }
...
fn run_indexer_step() {
	...
	// 		let count =
	// 			execute_indexer_save_step(&state.init.location, step, &ctx.library.clone()).await?;
